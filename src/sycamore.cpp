...
struct mt_error : std::runtime_error {
...
void SEM::ReadProbabilities() {
    std::ifstream inputFile(this->probabilityFileName.c_str());
    if (!inputFile) {
        throw mt_error("Failed to open probability file: " + this->probabilityFileName);
    }
	...
	// string node_parent_name;                 // persists across lines
    double prob; int i, j;
    ...
    while (std::getline(inputFile, line)) {
        ...
        switch (num_words) {
            ...
            case 16: {                
				...
				// SEM_vertex* p = this->GetVertex(node_parent_name);
				// n->parent = p;
...
void SEM::WriteProbabilities(string fileName) {
	...
	SEM_vertex * v;
	// probabilityFile << "probabilites are printed in following format" << endl;
...
void SEM::EM_root_search_at_each_internal_vertex_started_with_parsimony(int num_repetitions) {
	...
	for (int v_ind : vertex_indices_to_visit) {
		...
		for (int rep = 0; rep < num_repetitions; rep++) {					
			...
			loglikelihood_node_rep_file << v->name << "\t"
										...
										<< rep +1 << "\t"
										<< iter << "\t"
										<< setprecision(8) << logLikelihood_pars << "\t"
										<< setprecision(8) << loglikelihood_edc_first << "\t"
										<< setprecision(8) << loglikelihood_edc_final << "\t"
										<< setprecision(8) << logLikelihood_final << endl;
			if (max_log_likelihood < logLikelihood_final) {
				this->WriteProbabilities(this->prefix_for_output_files + ".probability_max_ll");
	...
	//  cout << "max log-likelihood is " << setprecision(8) << max_log_likelihood << endl;
	// (*this->logFile) << "max log-likelihood is " << setprecision(8) << max_log_likelihood << endl;
}
...
void SEM::EM_rooted_at_each_internal_vertex_started_with_parsimony(int num_repetitions) {
	...
	// cout << "maximum number of EM iterations allowed is " << this->maxIter << endl;
	// (* this->logFile) << "maximum number of EM iterations allowed is " << this->maxIter << endl;
    cout << "Writing probability to " << this->probabilityFileName << endl;
	int n = this->numberOfObservedVertices;
	int num_vertices = this->vertexMap->size();	
	SEM_vertex * v;
	vector <float> loglikelihoodscoresForEachRepetition;
	...
	for (int v_i = 0; v_i < vertex_indices_to_visit.size(); v_i++){
		...
		for (int rep = 0; rep < num_repetitions; rep++) {					
			...
			loglikelihood_node_rep_file << v->name << "\t"										
										...
										<< setprecision(8) << logLikelihood_final << endl;
			if (max_log_likelihood < logLikelihood_final) {                
				this->WriteProbabilities(this->probabilityFileName);
				max_log_likelihood = logLikelihood_final;
			}
		}
	}
...
tuple <int,double,double,double,double> SEM::EM_root_search_with_parsimony_rooted_at(SEM_vertex *v) {
	...
	this->ComputeInitialEstimateOfModelParameters();
	this->ComputeLogLikelihood();
	// cout << "Initial value of log-likelihood is " << setprecision(8) << this->logLikelihood << endl;
	// cout << "10d" << endl;
	this->ClearAncestralSequences();
	double logLikelihood_pars;
	...
	// cout << "-    -     -     -     -     -     -     -     -     -     -     -     -     -" << endl;
	// cout << "log-likelihood computed by marginalization using parsimony parameters is " << setprecision(10) << logLikelihood_pars << endl;
	// (*this->logFile) << "-    -     -     -     -     -     -     -     -     -     -     -     -     -" << endl;
	// (*this->logFile) << "log-likelihood computed by marginalization using parsimony parameters is " << setprecision(10) << logLikelihood_pars << endl;
	...
	while (continueIterations) {
		...
		// cout << "log-likelihood computed using expected counts after EM iteration " << iter << " is " << setprecision(10) << this->logLikelihood << endl;
		// (*this->logFile) << "log-likelihood computed using expected counts after EM iteration " << iter << " is " << setprecision(10) << this->logLikelihood << endl;
	...
	// cout << "log-likelihood computed by marginalization using EM parameters after iterations " << iter << " is " << setprecision(10) << this->logLikelihood << endl;
	// cout << "Root location selected by EM is " << this->root->name << endl;
	// cout << "- + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + -" << endl;
	// (*this->logFile) << "log-likelihood computed by marginalization using EM parameters after iterations " << iter << " is " << setprecision(10) << this->logLikelihood << endl;
...
tuple <int,double,double,double,double> SEM::EM_started_with_SSH_parameters_rooted_at(SEM_vertex *v) {
	...
	// cout << "-    -     -     -     -     -     -     -     -     -     -     -     -     -" << endl;
	// cout << "log-likelihood computed by marginalization using SSH parameters is " << setprecision(10) << logLikelihood_ssh << endl;
	// (*this->logFile) << "-    -     -     -     -     -     -     -     -     -     -     -     -     -" << endl;
	// (*this->logFile) << "log-likelihood computed by marginalization using SSH parameters is " << setprecision(10) << logLikelihood_ssh << endl;
	logLikelihood_exp_data_previous = -1 * pow(10,4);
	while (continueIterations) {
		// t_start_time = chrono::high_resolution_clock::now();
		iter += 1;
	...
	// cout << "log-likelihood computed by marginalization after EM iteration " << iter << " is " << setprecision(10) << this->logLikelihood << endl;
	// cout << "- + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + -" << endl;
	// (*this->logFile) << "log-likelihood computed by marginalization after EM iteration " << iter << " is " << setprecision(10) << this->logLikelihood << endl;
	// (*this->logFile) << "- + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + -" << endl;
...
tuple <int,double,double,double,double> SEM::EM_started_with_parsimony_rooted_at(SEM_vertex *v) {
	...
	// cout << "log-likelihood computed by marginalization using EM parameters " << iter << " is " << setprecision(10) << this->logLikelihood << endl;
	// cout << "- + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + -" << endl;
	// (*this->logFile) << "log-likelihood computed by marginalization using EM parameters " << iter << " is " << setprecision(10) << this->logLikelihood << endl;
	// (*this->logFile) << "- + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + - + -" << endl;
...
public:
	...
	void mstbackbone_k2020_preprint();
	...
	string EncodeAsDNA(vector<unsigned char> sequence);
	vector<unsigned char> DecompressSequence(vector<unsigned char>* compressedSequence, vector<vector<int>>* sitePatternRepeats);
	mstB(string sequenceFileNameToSet, string input_format, string topologyFileNameToSet, int num_repetitions, int max_iter, double conv_threshold, string probabilityFileName, string prefix_for_output_files) {	
        ...
        this->P->probabilityFileName = probabilityFileName;
...
void mstB::MSTBackboneOnlyLocalPhylo() {
	vector <string> names;
...
void mstB::mstbackbone_k2020_preprint() {
	vector <string> names;
	vector <vector <unsigned char> > sequences;
	vector <int> sitePatternWeights;
	vector <vector <int> > sitePatternRepetitions;	
...
void mstB::start_EMt_with_SSH_pars(int num_repetitions) {
	this->P->EM_rooted_at_each_internal_vertex_started_with_SSH_par(num_repetitions);	
}

void mstB::start_EMt_with_MPars(int num_repetitions){
	this->P->EM_rooted_at_each_internal_vertex_started_with_parsimony(num_repetitions);	
}
void mstB::MSTBackboneWithRootSEMAndMultipleExternalVertices() {
	vector <string> names;
	vector <vector <unsigned char> > sequences;
	vector <int> sitePatternWeights;
	vector <vector <int> > sitePatternRepetitions;	
...
void mstB::MSTBackboneWithOneExternalVertex() {
	this->P = new SEM(1,this->conv_thresh,this->max_EM_iter,this->verbose);
	cout << "Starting MST-backbone" << endl;
...
void mstB::EMgivenInputTopology(){
	vector <string> names;
	vector <vector <unsigned char> > sequences;
	vector <int> sitePatternWeights;
	vector <vector <int> > sitePatternRepetitions;		
...
vector<unsigned char> mstB::DecompressSequence(vector<unsigned char>* compressedSequence, vector<vector<int>>* sitePatternRepeats){
	int totalSequenceLength = 0;
	for (vector<int> sitePatternRepeat: *sitePatternRepeats){
		totalSequenceLength += int(sitePatternRepeat.size());
	}
...
string mstB::EncodeAsDNA(vector<unsigned char> sequence){
	string allDNA = "AGTC";
	string dnaSequence = "";
	for (unsigned char s : sequence){
		dnaSequence += allDNA[s];
...
string mstB::GetSequenceListToWriteToFile(map <string, vector <unsigned char>> compressedSeqMap, vector <vector <int> > sitePatternRepetitions) {	
	vector <unsigned char> decompressedSequence;
	string dnaSequence;
	string u_name;
...
int mstB::GetEdgeIndex (int vertexIndex1, int vertexIndex2, int numberOfVertices){
	int edgeIndex;
	edgeIndex = numberOfVertices*(numberOfVertices-1)/2;
	edgeIndex -= (numberOfVertices-vertexIndex1)*(numberOfVertices-vertexIndex1-1)/2;
	edgeIndex += vertexIndex2 - vertexIndex1 - 1;
...
int mstB::ComputeHammingDistance(string seq1, string seq2) {
	int hammingDistance = 0;
...
int mstB::ComputeHammingDistance(vector<unsigned char> recodedSeq1, vector<unsigned char> recodedSeq2) {
	int hammingDistance = 0;
...
PYBIND11_MODULE(emaxim, m) {
    m.doc() = "pybind11 bindings for emaxim";
    m.def("ord_pair", &ord_pair);
    py::class_<mstB>(m, "mstB")
        .def(py::init<
                 std::string,  // sequence_file
                 std::string,  // seq_file_format
                 std::string,  // topology_file
                 int,          // num_repetitions
                 int,          // max_iter
                 double,       // conv_threshold
                 std::string,  // probability_file
                 std::string   // prefix_for_output_files
             >(),
             ...
             py::arg("probability_file"),
             py::arg("prefix_for_output"))
        .def("EMpars", &mstB::EMpars)        
        .def("EMssh", &mstB::EMssh);
